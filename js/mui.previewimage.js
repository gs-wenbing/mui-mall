(function($, window) {
	...
	var PreviewImage = function(options) {
		this.options = $.extend(true, {
			...
			zoom: true,
			header: '<span class="mui-preview-indicator" style="margin-top:40px"></span>',
			footer: '<div style="text-align: center;"><span id="saveImg" style="background: rgba(0,0,0,0.5) !important;border-radius:5px;padding: 5px 20px;color:#ffffff;line-height: 30px;height: 30px;vertical-align: middle;">保存图片</span></div>'
		}, options || {});
		this.init();
		this.initEvent();
	};
	...
	proto.initEvent = function() {
		...
		$(document.body).on('tap', '#saveImg', function() {
			var wd = plus.nativeUI.showWaiting("正在下载...");
			var itemEl = self.scroller.querySelector($.classSelector('.slider-item:nth-child(' + (self.lastIndex + 1) + ')'));
			var imgEl = itemEl.querySelector('img');
			var imgDtask = plus.downloader.createDownload(imgEl.src, {
				method: 'GET'
			}, function(d, status) {
				if (status == 200) {
